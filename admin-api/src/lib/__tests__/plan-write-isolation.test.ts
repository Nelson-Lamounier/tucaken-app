...
// Root of admin-api/src - this file lives at src/lib/__tests__/plan-write-isolation.test.ts
 ...
 */
const ALLOWED_ROUTE = 'billing/internal-billing.ts';

/**
 * Return all .ts files under src/routes/ (recursive — routes live in
 * domain subfolders), excluding .test.ts.
 */
  ...
  for (const identifier of PLAN_WRITE_IDENTIFIERS) {
    it(`"${identifier}" is referenced only by ${ALLOWED_ROUTE} among route files`, () => {
      ...
      for (const filePath of routeFiles) {
        const relPath = path
          .relative(path.join(SRC_ROOT, 'routes'), filePath)
          .split(path.sep)
          .join('/');
        if (relPath === ALLOWED_ROUTE) continue; // the permitted writer
