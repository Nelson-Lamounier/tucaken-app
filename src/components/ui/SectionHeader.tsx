...
export function SectionHeader({ 
  ...
  return (
    <div 
      className={`border-b border-white/10 px-4 py-5 sm:px-6 ${isClickable ? 'cursor-pointer hover:bg-white/[0.02] transition-colors group focus:outline-none focus:bg-white/[0.03]' : ''}`}
      onClick={onClick}
    ...
    >
      <div className="-mt-4 -ml-4 flex flex-wrap items-center justify-between sm:flex-nowrap">
        <div className="mt-4 ml-4 flex items-center gap-3">
          ...
          <div>
            <h3 className={`text-base font-semibold transition-colors ${isClickable ? 'text-indigo-400 group-hover:text-indigo-300' : 'text-white'}`}>
              {title}
            </h3>
            {description && (
