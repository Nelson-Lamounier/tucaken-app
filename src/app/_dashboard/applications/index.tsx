...
import {
  GridListActions,
  type GridListAction,
  type GridListActionGroup,
} from '@/components/ui/GridListActions'
import { DashboardPage } from '@/components/layouts/DashboardPage'

const newAnalysisAction: GridListAction = {
  title: 'New Analysis',
...
const allApplicationsAction: GridListAction = {
...
const prepareForInterviewAction: GridListAction = {
...
// Non-admins see the card but cannot act on it yet.
function buildCoreWorkflowGroup(isAdmin: boolean): GridListActionGroup {
  const prepare: GridListAction = isAdmin
    ? prepareForInterviewAction
    : { ...prepareForInterviewAction, href: undefined, disabled: true, badge: 'Coming soon' }

  return {
    label: 'Core workflow',
    actions: [newAnalysisAction, allApplicationsAction, prepare],
  }
}

const githubRepositoriesAction: GridListAction = {
...
// Admin-only article tooling. Hidden for non-admins here, and the /ai-agent
// route enforces the same check server-side (hidden UI is not access control).
const adminArticleActions: GridListAction[] = [
  {
...
function ApplicationsIndexRoute() {
  ...
  const knowledgeActions = isAdmin
    ? [githubRepositoriesAction, ...adminArticleActions]
    : [githubRepositoriesAction]

  const actionGroups: GridListActionGroup[] = [
    buildCoreWorkflowGroup(isAdmin),
    { label: 'Knowledge & content', actions: knowledgeActions },
  ]
