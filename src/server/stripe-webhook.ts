...
/**
 * Claims the event id via admin-api (`webhook_events_seen` table); returns
 * true when this is a duplicate delivery that should be skipped.
 *
 * Fails OPEN: if the dedupe call errors (transient network, table missing),
 * we return false and process the event anyway. The conditional writes in the
 * subscription sync remain the safety net — dropping a real billing event
 * would be worse than a rare reprocess.
 */
