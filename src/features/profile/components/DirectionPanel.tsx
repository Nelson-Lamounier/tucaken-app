...
const TIER = {
  strong:   'border-teal-500/30 bg-teal-500/10 text-teal-700 dark:text-teal-300',
  moderate: 'border-amber-500/20 bg-amber-500/10 text-amber-700 dark:bg-amber-500/8 dark:text-amber-300',
  weak:     'border-zinc-200 bg-zinc-50 text-zinc-500 dark:border-white/10 dark:bg-white/5',
} as const
    ...
    <section className="flex flex-col gap-4">
      ...
      {d.seniority.length > 0 && (
        <ul className="space-y-1 text-xs text-zinc-700 dark:text-zinc-300">
          {d.seniority.map(s => (
            <li key={s.area}><span className="text-zinc-900 dark:text-zinc-100">{s.area}</span> — {s.level}
        ...
        <div>
          ...
          {open && (
            <ul className="mt-2 list-disc space-y-1 pl-4 text-xs text-zinc-700 dark:text-zinc-300">
              {d.whatToDeepen.map(w => <li key={w}>{w}</li>)}
            </ul>
          )}
        </div>
      )}
