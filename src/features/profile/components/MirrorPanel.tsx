  ...
  return (
    <section className="flex flex-col gap-4">
      {summary.mirror?.paragraph
        ? <p className="text-sm leading-relaxed text-zinc-700 dark:text-zinc-200">{summary.mirror.paragraph}</p>
        : <p className="text-sm text-zinc-500">Your profile summary is still being generated.</p>}

      <div className="flex flex-wrap gap-2">
        {langs.map(l => (
          <span key={l.language} className="rounded border border-zinc-200 bg-zinc-50 px-1.5 py-0.5 text-[10px] text-zinc-600 dark:border-white/10 dark:bg-white/5 dark:text-zinc-400">
            {l.language} {Math.round(l.sharePct)}%
          </span>
        ))}
        {rollup?.domains?.dominant && (
          <span className="rounded border border-indigo-500/20 bg-indigo-500/10 px-1.5 py-0.5 text-[10px] text-indigo-700 dark:bg-indigo-500/8 dark:text-indigo-300">
            {rollup.domains.dominant}
          </span>
        )}
      ...
      {arc.length > 0 && (
        <div className="flex items-center gap-1">
          {arc.map((p, i) => (
            <span key={`${p.date}-${p.domain}-${i}`} title={`${p.date} · ${p.language ?? '—'} · ${p.domain}`}
              className="h-1.5 w-3 rounded-sm bg-teal-500/40 dark:bg-teal-400/40" />
          ))}
        </div>
      )}
        ...
        <div>
          ...
          {open && (
            <ul className="mt-2 list-disc space-y-1 pl-4 text-xs text-zinc-700 dark:text-zinc-300">
              {reveals.map(r => (
                <li key={r.insight}>{r.insight} <span className="text-zinc-500">({r.evidence})</span></li>
              ))}
            </ul>
          )}
        </div>
